...
**Backend layering**: `controller → service → repository`, plus `client/` for outbound HTTP integrations (CoinGecko, RSS news feeds, OpenRouter) and `security/` for JWT. Entities are never serialized directly — every endpoint has a request/response DTO in `dto/`.
...
client/       outbound HTTP wrappers — CoinGeckoClient, RssNewsClient, OpenRouterClient
